...
# TODO: fix evaluation strategy data collection  into parquet file should be streamed, or switch to redis/database.
"""
...
def merge_data(input_df: pd.DataFrame, output_df: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = pd.merge(
        input_df, output_df, on=merge_keys, how="inner", suffixes=("_input", "_output")
    )
...
def compute_metrics(
    merged_df: pd.DataFrame, ground_truth_df: pd.DataFrame, threshold_strategies: dict
) -> dict:
...
def plot_roc_curve(
    merged_df: pd.DataFrame, ground_truth_df: pd.DataFrame, output_dir: str
):
...
def plot_score_distribution(merged_df: pd.DataFrame, output_dir: str):
    ...
    axes[1].hist(
        merged_df["z_score"], bins=50, alpha=0.7, edgecolor="black", color="orange"
    )
    axes[1].axvline(
        2, color="yellow", linestyle="--", linewidth=2, label="z=2 (medium)"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate RRCF model performance")
    parser.add_argument(
        "--data-dir",
        required=True,
    ...
    parser.add_argument(
        "--ground-truth", help="Path to ground truth anomalies JSON file (optional)"
    )
    parser.add_argument(
        "--output-dir", required=True, help="Directory to save evaluation results"
    )
