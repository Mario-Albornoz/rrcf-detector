...
class TestTreeCreation:
    ...
    def test_multiple_keys_create_separate_trees(self, detector):
        ...
        vector1 = NormalizedVectorDto(
            exchange="binance", instrument="BTC-USDT", instrument_class="crypto_spot",
            timestamp=datetime.now(), model_key="test",
            z_intertick_fast=1.0, z_price_step_fast=1.0, z_intertick_slow=1.0,
            z_price_step_slow=1.0, cusum_intertick=1.0, cusum_price_step=1.0,
            gap_flag=0, warmup_flag=0, session_fallback_flag=0
        )
        
        vector2 = NormalizedVectorDto(
            exchange="coinbase", instrument="BTC-USD", instrument_class="crypto_spot",
            timestamp=datetime.now(), model_key="test",
            z_intertick_fast=1.0, z_price_step_fast=1.0, z_intertick_slow=1.0,
            z_price_step_slow=1.0, cusum_intertick=1.0, cusum_price_step=1.0,
            gap_flag=0, warmup_flag=0, session_fallback_flag=0
        ...
        vector3 = NormalizedVectorDto(
            exchange="binance", instrument="BTC-PERP", instrument_class="crypto_futures",
            timestamp=datetime.now(), model_key="test",
            z_intertick_fast=1.0, z_price_step_fast=1.0, z_intertick_slow=1.0,
            z_price_step_slow=1.0, cusum_intertick=1.0, cusum_price_step=1.0,
            gap_flag=0, warmup_flag=0, session_fallback_flag=0
    ...
    def test_oldest_point_evicted_on_overflow(self, detector):
        ...
        for i in range(window_size + 2):
            vector = NormalizedVectorDto(
                exchange="binance", instrument="BTC-USDT", instrument_class="crypto_spot",
                timestamp=datetime.now(), model_key="test",
                z_intertick_fast=float(i), z_price_step_fast=float(i),
                z_intertick_slow=float(i), z_price_step_slow=float(i),
                cusum_intertick=float(i), cusum_price_step=float(i),
                gap_flag=0, warmup_flag=0, session_fallback_flag=0
            )
    ...
    def test_normal_points_have_similar_scores(self, detector):
        ...
        for i in range(5):
            vector = NormalizedVectorDto(
                exchange="binance", instrument="BTC-USDT", instrument_class="crypto_spot",
                timestamp=datetime.now(), model_key="test",
                z_intertick_fast=1.0 + i * 0.1, z_price_step_fast=0.5 + i * 0.1,
                z_intertick_slow=1.0 + i * 0.1, z_price_step_slow=0.5 + i * 0.1,
                cusum_intertick=2.0, cusum_price_step=1.0,
                gap_flag=0, warmup_flag=0, session_fallback_flag=0
            )
    ...
    def test_anomalous_point_has_higher_score(self, detector):
        ...
        for i in range(5):
            normal_vector = NormalizedVectorDto(
                exchange="binance", instrument="BTC-USDT", instrument_class="crypto_spot",
                timestamp=datetime.now(), model_key="test",
                z_intertick_fast=1.0, z_price_step_fast=0.5,
                z_intertick_slow=1.0, z_price_step_slow=0.5,
                cusum_intertick=2.0, cusum_price_step=1.0,
                gap_flag=0, warmup_flag=0, session_fallback_flag=0
            )
        ...
        anomaly_vector = NormalizedVectorDto(
            exchange="binance", instrument="BTC-USDT", instrument_class="crypto_spot",
            timestamp=datetime.now(), model_key="test",
            z_intertick_fast=100.0, z_price_step_fast=100.0,  # Extreme values
            z_intertick_slow=100.0, z_price_step_slow=100.0,
            cusum_intertick=100.0, cusum_price_step=100.0,
            gap_flag=0, warmup_flag=0, session_fallback_flag=0
        )
